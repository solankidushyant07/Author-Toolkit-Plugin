...
export class AuthorToolkitSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.empty();
		containerEl.createEl("h2", { text: "Author Toolkit Settings" });

		new Setting(containerEl)
			.setName("Templates Folder Path")
			.setDesc("The default folder where your templates are stored. (e.g., Templates/Characters)")
			.addText((text) => text
		...
		containerEl.createEl("h3", { text: "Creation Defaults" });
		...
		new Setting(containerEl)
			...
			.setDesc("What Create Notes should do when a note with the same name already exists.")
			.addDropdown((dropdown) => dropdown
		...
		new Setting(containerEl)
			...
			.setDesc("What Create Folders should do when a folder with the same name already exists.")
			.addDropdown((dropdown) => dropdown
				.addOption("rename", "Rename (Add number)")
				.addOption("skip", "Skip it")
		...
		new Setting(containerEl)
			.setName("Version")
			.setDesc("Current Development Version")
			.addExtraButton((button) =>
				button
					.setIcon("info")
					.setTooltip("Current Version")
			);
